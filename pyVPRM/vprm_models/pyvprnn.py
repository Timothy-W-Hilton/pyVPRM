...
class pyvprnn:
    ...
    def prepare_base_dataset(
        self, nee_qc_flags=[0], opath=None, mass_fraction_threshold=0.925, chunks_t=256
    ):
        ...
        counts = xr.concat(
            [(scl_valid == c).sum(dim="time") for c in valid_classes], dim="class"
        ...
        common_times = valid_times.sel(datetime_utc=valid_times.isin(fp_times))

        footprint_sum = (
            ds_cropped["ffp_footprint"].sel(t=common_times).sum(dim=["x", "y"])
        ).compute()
    ...
    def prepare_base_dataset_old(
        self, nee_qc_flags=[0], opath=None, mass_fraction_threshold=0.925
    ):
        ...
        self.ds_cropped["ffp_footprint"] = (
            self.ds_cropped["ffp_footprint"] / spatial_sum
        )

        valid_footprint_mask = (
            self.ds_cropped["ffp_footprint"].sum(dim=["x", "y"], skipna=True) != 0
        )
        vars_datetime = [
            v for v in self.ds.data_vars if "datetime_utc" in self.ds[v].dims
        ]
        ...
        common_times = valid_times.sel(
            datetime_utc=valid_times.isin(
                self.ds_cropped["ffp_footprint"].where(valid_footprint_mask, drop=True)[
                    "t"
                ]
            )
        )
        ...
        footprint_sum = (
            self.ds_cropped["ffp_footprint"].sel(t=common_times).sum(dim=["x", "y"])
        )
    ...
    def split_train_val_test(self, k=5, val_frac=0.15, test_frac=None, random_state=42):
        """
    ...
    def split_train_val_test_years(
        self,
        num_test_years=1,
        test_year=None,
        val_size=0.2,
        ...
        train_days, val_days = train_test_split(
            unique_days, test_size=val_size, random_state=random_state
        )
    ...
    def balance_subset(self, times_subset, random_state=41):
        ...
        df = pd.DataFrame({"time": times_subset, "t2m": t2m_vals})

        # --- define bins for t2m ---
        bins_fixed = pd.cut(
            df["t2m"], bins=np.arange(-2, 31, 4)
        )  # e.g., -2 to 30 °C in 4°C bins
        max_count = df.groupby(bins_fixed).size().max()
        max_factor = 2  # allow each row to appear up to 2x

        # --- oversample within bins ---
        df_balanced = df.groupby(bins_fixed, group_keys=False).apply(
            lambda x: x.sample(
    ...
    def compute_sample_weights_t2m_vpd_global(
        ...
        vpd_edges = np.linspace(
            df_train["vpd"].min(), df_train["vpd"].max(), n_bins + 1
        )
    ...
    def get_data_for_upscaling(
        self, vprm_pre=None, met=None, datetimes=None, base_path=None, meteo_vars=None
    ):
        self.era5_inst = met
        ...
        self.era5_inst.ds_out["vpd_decorrelated_log"] = np.log(
            np.maximum(self.era5_inst.ds_out["vpd"], 0.01)
        ) - np.log(es)
        for key in self.era5_inst.ds_out.keys():
            print(key)
            self.ds[key + "_era5"] = self.era5_inst.ds_out[key].sel(
                {"valid_time": dateteims}, method="nearest"
            )
        sat_vars = ["lswi", "evi", "nirv", "ndre"]
        self.ds[sat_vars] = self.ds[sat_vars].fillna(0.0)
    ...
    def get_training_data(
        ...
        for key in flux_tower_keys:
            try:
                self.ds[key] = xr.DataArray(
                    ...
                    dims=("datetime_utc",),
                    coords={"datetime_utc": self.flux_tower.flux_data["datetime_utc"]},
        ...
        footprint_timestamps = self.ds["datetime_utc"].where(mask, drop=True)

        footprints = []
        for i, chunk_of_timestamps in enumerate(
            np.array_split(footprint_timestamps, 100)
        ):
            print(i)
            ...
            self.ffp_handler.regrid_calculation_grid_to_satellite_grid(
                vprm_pre.sat_imgs.sat_img, base_path
            )
            footprints.append(
                self.ffp_handler.footprint_on_satellite_grid["footprint"].astype(
                    "float32"
                )
            )
        ...
        self.era5_inst.reduce_time(
            self.flux_tower.flux_data["datetime_utc"].iloc[0],
        ...
        for key in self.era5_inst.ds_out.keys():
            print(key)
            self.ds[key + "_era5"] = self.era5_inst.ds_out[key].sel(
                {"valid_time": self.ds["datetime_utc"]}, method="nearest"
            )
    ...
    def make_satellite_animation(self, opath=None):
        ...
        transformer = Transformer.from_crs(
            "EPSG:4326", self.ds.rio.crs, always_xy=True  # or "EPSG:32632"
        )
        ...
        ani = FuncAnimation(
            fig, update, frames=ntime, interval=150, blit=True  # ms between frames
        )

        if opath is not None:
            ani.save(opath, writer=PillowWriter(fps=6))
        ...
        for j, val in enumerate(f_values):
            ...
            if X_met_lagged is not None:
                y_pred = self.pixel_model.predict(
                    [
                        X_sat_c,
                        X_met_tmp[:, np.newaxis, np.newaxis, :],
                        X_met_lagged_c,
                        X_mask_c,
                    ],
                    verbose=0,
    ...
    def plot_ice_pdp(
        ...
        if show_ices:
            for i in range(ice.shape[0]):
                ax.plot(
                    f_values, ice[i], color=colors[i], linewidth=0.4, alpha=ice_alpha
                )
        ...
        if show_band:
            ...
            ax.fill_between(
                f_values, p5, p95, color="gray", alpha=0.3, label="5–95% ICE range"
            )
        ...
        # -------------------------
        ax.plot(
            f_values, pdp, color="black", linewidth=1, label="Median PDP", zorder=10
        )
