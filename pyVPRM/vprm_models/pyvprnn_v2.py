...
class pyvprnn_v2(pyvprnn):
    ...
    def build_nn_arrays(self, times, met_dim=1, subsample_step=4):
        ...
        flux_mask = np.repeat(
            self.ds_cropped["class4_ge10pct"].values[None, ...], T, axis=0
        ).astype(np.float32)
    ...
    def train(
        ...
        (
            ...
            flux_mask_train,
            y_train,
        ) = self.build_nn_arrays(
            train_times_qc0, met_dim=1
        )  #
        ...
        gpp_met_idx = [
            i for i in range(n_met_features) if i not in [swvl1_idx, skt_idx]
        ]  # skt_idx
        filter_size = 1
        ...
        # =========================================================
        temp_swvl_idx = [
            self.lagged_met_vars.index("t2m"),
            self.lagged_met_vars.index("swvl1_era5"),
            self.lagged_met_vars.index("stl2_era5"),
        ]
        x_met_lagged = SelectFeatures(temp_swvl_idx, name="select_temp_swvl")(
        ...
        x_met_lagged = layers.Dense(
            3, activation="softplus", name="dense_lagged_summary"
        )(x_met_lagged[:, -1, :])

        met_bc_lagged = BroadcastToImage(name="met_bc_lagged")(
            [x_met_lagged, self.sat_input]
        )
        ...
        x_reco = layers.Concatenate(name="reco_concat")(
            [self.sat_input, met_bc_reco, met_bc_lagged]
        )
        ...
        history = self.model.fit(
            ...
            y=y_train,
            validation_data=(
                [Xsat_test, Xmet_test, Xmet_windowed_test, fp_test, flux_mask_test],
                y_test,
            ),  #  self.valid_weights
