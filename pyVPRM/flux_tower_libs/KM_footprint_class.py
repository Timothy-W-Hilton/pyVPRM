...
class KM_footprint_manager(base_footprint_manager):
    def __init__(
        ...
        time_stamps,
        flux_tower_manager,
        calculation_grid_side_length,
        calculation_grid_pixels_per_side,
    ):
        super().__init__(
            time_stamps,
            flux_tower_manager,
    ...
    def calculate_footprints(self):
        ...
        stability_c = np.where(
            self.ZL > 0, (1 + 5 * self.ZL), (1 - 16 * self.ZL) ** (-1 / 2)
        )
        stability_m = np.where(
            self.ZL > 0, (1 + 5 * self.ZL), (1 - 16 * self.ZL) ** (-1 / 4)
        )
        n = np.where(
            self.ZL > 0, 1 / (1 + 5 * self.ZL), (1 - 24 * self.ZL) / (1 - 16 * self.ZL)
        )
        ...
        crosswind_integrated_flux = (
            np.expand_dims(xi**mu / gamma(mu), axis=(1, 2))
            / self.X_calculation_grid_rotated ** np.expand_dims((1 + mu), axis=(1, 2))
            * np.exp(-np.expand_dims(xi, axis=(1, 2)) / self.X_calculation_grid_rotated)
        )
        if not np.all(
            np.logical_or(
                np.isnan(crosswind_integrated_flux), crosswind_integrated_flux == 0
            )
        ):
            crosswind_integrated_flux = np.where(
                np.isnan(crosswind_integrated_flux), 0, crosswind_integrated_flux
            )

        # calculate crosswind_distribution
        u_bar = np.expand_dims(
            gamma(mu) / gamma(1 / r) * (r**2 * kappa / U) ** (m / r) * U, axis=(1, 2)
        ) * self.X_calculation_grid_rotated ** np.expand_dims((m / r), axis=(1, 2))
        sigma_y = (
            self.X_calculation_grid_rotated
            * np.expand_dims(self.sigma_v, axis=(1, 2))
            / u_bar
        )
        crosswind_distribution = (
            1
            / ((2 * np.pi) ** 0.5 * sigma_y)
            * np.exp(-self.Y_calculation_grid_rotated**2 / (2 * sigma_y**2))
        )
        # if not np.all(np.logical_or(np.isnan(crosswind_distribution), crosswind_distribution==0)):
        crosswind_distribution = np.where(
            np.isnan(crosswind_distribution), 0, crosswind_distribution
        )

        # calculate footprints:
        footprint = crosswind_distribution * crosswind_integrated_flux
        integrated_footprint = (
            np.nansum(footprint, axis=(1, 2)) * self.side_length_pixel**2
        )
    ...
    def calculate_crosswind_integrated_flux(self):
        ...
        # calculate crosswind_integrated_flux
        crosswind_integrated_flux = (
            np.expand_dims(xi**mu / gamma(mu), axis=(1))
            / self.xs_for_calculation_grid ** np.expand_dims((1 + mu), axis=(1))
            * np.exp(-np.expand_dims(xi, axis=(1)) / self.xs_for_calculation_grid)
        )
        if not np.all(
            np.logical_or(
                np.isnan(crosswind_integrated_flux), crosswind_integrated_flux == 0
