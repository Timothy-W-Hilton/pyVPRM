...
def make_xesmf_grid_from_satellite_image(satellite_image):
    ...
    my_transformer = Transformer.from_crs(
        satellite_image.rio.crs, "+proj=longlat +datum=WGS84", always_xy=True
    )

    # apply transformer to the grids
    X_center_grid, Y_center_grid = my_transformer.transform(
        X_center_grid, Y_center_grid
    )
    X_corner_grid, Y_corner_grid = my_transformer.transform(
...
def make_xesmf_grid_from_x_y(footprint_data, lon, lat, elev):
    ...
    # make meshgrids
    X_center_grid, Y_center_grid = np.meshgrid(
        x_pixel_centers, y_pixel_centers, indexing="ij"
    )
    X_corner_grid, Y_corner_grid = np.meshgrid(
        x_pixel_corners, y_pixel_corners, indexing="ij"
    )

    # define transformer to transform from the images crs to the defined crs of WGS84
    my_transformer = Transformer.from_pipeline(
        "+proj=pipeline +step +inv +proj=topocentric +lon_0={0} +lat_0={1} +h_0={2} +step +inv +proj=cart +ellps=WGS84".format(
            lon, lat, elev
        )
    )
    ...
    # apply transformer to the grids
    X_center_grid, Y_center_grid = my_transformer.transform(
        X_center_grid, Y_center_grid, errcheck=True
    )
    X_corner_grid, Y_corner_grid = my_transformer.transform(
        X_corner_grid, Y_corner_grid, errcheck=True
    )
...
class base_footprint_manager:
    def __init__(
        ...
        if (self.calculation_grid_side_length is not None) and (
            self.calculation_grid_resolution is not None
        ):
            self.side_length_pixel = (
                self.calculation_grid_side_length / self.calculation_grid_resolution
            )
    ...
    def set_calculation_grid_side_length_and_resolution(
        self, calculation_grid_side_length, calculation_grid_resolution
    ):
    ...
    def setup(self):
        flux_tower_data = self.flux_tower_manager.flux_data.where(
            self.flux_tower_manager.flux_data != -9999.0
        )  # set invalid values to nan, don't drop
        ...
        flux_tower_data["datetime_utc"] = pd.to_datetime(
            flux_tower_data["datetime_utc"], utc=True
        )
        # print(t_Frame)
        # print(self.flux_tower_manager.flux_data.datetime_utc)
        flux_tower_data = pd.merge(
            flux_tower_data, t_Frame, on="datetime_utc", how="right"
        )  # merge on t_frame
        ...
        if "PBLH" in flux_tower_data.columns:
            ...
            self.era5_instance.get_data_series(
                self.flux_tower_manager.get_lonlat(), "blh", self.time_stamps
            )
        else:
    ...
    def make_calculation_grid(self, recalculate=True):
        ...
        if (self.X_calculation_grid_rotated is None) or recalculate:
            self.xs_for_calculation_grid = np.linspace(
                -0.5 * (self.calculation_grid_side_length - self.side_length_pixel),
                0.5 * (self.calculation_grid_side_length - self.side_length_pixel),
                self.calculation_grid_resolution,
            )
            self.ys_for_calculation_grid = np.linspace(
                -0.5 * (self.calculation_grid_side_length - self.side_length_pixel),
                0.5 * (self.calculation_grid_side_length - self.side_length_pixel),
                self.calculation_grid_resolution,
            )
            self.X_calculation_grid, self.Y_calculation_grid = np.meshgrid(
                self.xs_for_calculation_grid, self.ys_for_calculation_grid
            )

            # rotate grid so that the x axis vector is (anti)parallel to the mean wind vector
            self.X_calculation_grid_rotated = np.expand_dims(
                np.cos((-self.u_dir + 90) * np.pi / 180), axis=(1, 2)
            ) * np.expand_dims(self.X_calculation_grid, axis=0) + np.expand_dims(
                np.sin((-self.u_dir + 90) * np.pi / 180), axis=(1, 2)
            ) * np.expand_dims(
                self.Y_calculation_grid, axis=0
            )
            self.Y_calculation_grid_rotated = -np.expand_dims(
                np.sin((-self.u_dir + 90) * np.pi / 180), axis=(1, 2)
            ) * np.expand_dims(self.X_calculation_grid, axis=0) + np.expand_dims(
                np.cos((-self.u_dir + 90) * np.pi / 180), axis=(1, 2)
            ) * np.expand_dims(
    ...
    def regrid_calculation_grid_to_satellite_grid(
        self, satellite_grid, regridder_file_path
    ):
        # print('regridd to satellite grid')
        footprint_to_satellite_file_path = os.path.join(
            regridder_file_path,
            "{0}_footprint_{1}_px_per_{2}_m_to_sentinel2_{3}_x_{4}_px.nc".format(
                self.site_ID,
                self.calculation_grid_resolution,
                self.calculation_grid_side_length,
                len(satellite_grid.x.values),
                len(satellite_grid.y.values),
            ),
        )
        my_transformer = Transformer.from_pipeline(
            "+proj=pipeline +step +inv +proj=topocentric +lon_0={0} +lat_0={1} +h_0={2} +step +inv +proj=cart +ellps=WGS84".format(
                self.lon, self.lat, self.elev + self.z_displacement
            )
        )
        xesmf_grid_footprint = make_xesmf_grid(
            self.footprint_on_calculation_grid, transformer=my_transformer
        )
        ...
        if os.path.exists(footprint_to_satellite_file_path):
            ...
            self.regridder = xe.Regridder(
                xesmf_grid_footprint,
                xesmf_grid_satellite,
            ...
            self.regridder = xe.Regridder(
                xesmf_grid_footprint, xesmf_grid_satellite, method="conservative_normed"
            )
            print("Store new regridder")
            self.regridder.to_netcdf(footprint_to_satellite_file_path)
        self.footprint_on_satellite_grid = self.regridder(
            self.footprint_on_calculation_grid
        )
        self.footprint_on_satellite_grid = (
            self.footprint_on_satellite_grid.assign_coords(
                {
        ...
        summed_footprint = self.footprint_on_satellite_grid.footprint.sum(
            dim=["x", "y"]
        )
        self.footprint_on_satellite_grid["footprint"] = (
            self.footprint_on_satellite_grid.footprint
            / summed_footprint.where(summed_footprint != 0)
        )
        self.footprint_on_satellite_grid["integrated"] = (
            self.footprint_on_calculation_grid.integrated
        )
        self.footprint_on_satellite_grid["integrated_on_sat"] = (
            self.footprint_on_satellite_grid.footprint.sum(dim=["x", "y"])
        )
        self.footprint_on_satellite_grid["x_peak"] = (
            self.footprint_on_calculation_grid.x_peak
        )
        if self.footprint_model == "FFP":
            self.footprint_on_satellite_grid["x_70"] = (
                self.footprint_on_calculation_grid.x_70
            )
            self.footprint_on_satellite_grid["x_90"] = (
                self.footprint_on_calculation_grid.x_90
            )
    ...
    def get_radius_of_percentile(self, percentile=0.9):
        ...
        for idx_time in range(len(self.crosswind_integrated_flux[:, 0])):
            for idx_flux, flux in enumerate(
                self.crosswind_integrated_flux[idx_time, :]
            ):
    ...
    def get_radius_peak(self):
        r_peak = self.xs_for_calculation_grid[
            np.argmax(self.crosswind_integrated_flux, axis=1)
        ]
    ...
    def show_footprints_on_calculation_grid(self, output_image_path):
        ...
        def update(i):
            fig.suptitle(
                "{0} footprint on calculation grid at {1}, t={2}, u={3}m/s, \n u_dir={4}°, ZL={5}, u_star={6}m/s,  sigma_v={7}m/s".format(
            ...
            # replot
            CS = ax.contour(
                self.X_calculation_grid,
                self.Y_calculation_grid,
                footprint["footprint"].values,
                cmap="spring",
            )
            if not np.isnan(footprint["x_peak"]):
                x_peak = (
                    -np.cos((self.u_dir[i] + 90) * np.pi / 180) * footprint["x_peak"]
                )
                y_peak = (
                    np.sin((self.u_dir[i] + 90) * np.pi / 180) * footprint["x_peak"]
                )
                ax.scatter(x_peak, y_peak)
                if self.footprint_model == "FFP":
                    x_70 = (
                        -np.cos((self.u_dir[i] + 90) * np.pi / 180) * footprint["x_70"]
                    )
                    y_70 = (
                        np.sin((self.u_dir[i] + 90) * np.pi / 180) * footprint["x_70"]
                    )
                    x_90 = (
                        -np.cos((self.u_dir[i] + 90) * np.pi / 180) * footprint["x_90"]
                    )
                    y_90 = (
                        np.sin((self.u_dir[i] + 90) * np.pi / 180) * footprint["x_90"]
                    )
            ...
            ax.arrow(
                0,
                0,
                -np.sin(self.u_dir[i] * np.pi / 180) * 20 * self.u[i],
                -np.cos(self.u_dir[i] * np.pi / 180) * 20 * self.u[i],
                width=3,
            )

        ani = animation.FuncAnimation(
            fig, update, frames=len(self.time_stamps), interval=1000
        )
        ani.save(
            os.path.join(
                output_image_path,
                "{}_{}_footprint_evolution_on_meshgrid.gif".format(
                    self.site_ID, self.footprint_model
                ),
            ),
            writer="pillow",
        )
    ...
    def show_footprints_on_satellite_grid(self, output_image_path, X, Y):
        ...
        def update(i):
            fig.suptitle(
                "{0} footprint on satellite grid at {1}, t={2}, u={3}m/s, \n u_dir={4}°, ZL={5}, u_star={6}m/s,  sigma_v={7}m/s".format(
                    self.footprint_model,
                    self.site_ID,
                    self.time_stamps[i],
                    self.u[i],
        ...
        ani.save(
            os.path.join(
                output_image_path,
                "{}_{}_footprint_evolution_on_satellite_grid.gif".format(
                    self.site_ID, self.footprint_model
                ),
            ),
            writer="pillow",
    ...
    def show_regridded_footprints(self, output_image_path):
        ...
        ani.save(
            os.path.join(
                output_image_path,
                "{}_{}_footprint_regridded_evolution.gif".format(
                    self.site_ID, self.footprint_model
                ),
            ),
            writer="pillow",
    ...
    def show_footprint_percentiles(self, output_image_path, X, Y):
        ...
        def update(i):
            fig.suptitle(
                "{0} footprint 68th, 90th, 95th percentile at {1}, t={2}, u={3}m/s, \n u_dir={4}°, ZL={5}, u_star={6}m/s,  sigma_v={7}m/s".format(
                    self.footprint_model,
                    self.site_ID,
                    self.time_stamps[i],
                    self.u[i],
            ...
            integral = (
                (footprint["footprint"].values >= t[:, None, None])
                * footprint["footprint"].values
            ).sum(axis=(1, 2))
            ...
            # replot
            CS = ax.contour(
                X, Y, footprint["footprint"].values, t_contours, cmap="spring"
            )
            if not np.isnan(footprint["x_peak"]):
                x_peak = (
                    -np.cos((self.u_dir[i] + 90) * np.pi / 180) * footprint["x_peak"]
        ...
        ani = animation.FuncAnimation(
            fig, update, frames=len(self.time_stamps), interval=1
        )
        ani.save(
            os.path.join(
                output_image_path,
                "{}_{}_footprint_percentiles.gif".format(
                    self.site_ID, self.footprint_model
                ),
            ),
            writer="pillow",
