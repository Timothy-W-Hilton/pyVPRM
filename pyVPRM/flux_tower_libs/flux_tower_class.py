...
class icos(flux_tower_data):
    def __init__(
        ...
        if self.icos_2025:
            self.site_info = pd.read_csv(
                os.path.join(
                    os.path.dirname(self.data_path).replace(
                        "FLUXNET_HH_L2", "ARCHIVE_L2"
                    ),
            ...
            if "FETCH_50" in self.footprint.keys():
                self.fetch50 = np.percentile(
                    self.footprint["FETCH_50"][self.footprint["FETCH_50"] > 0], 90
                )
            else:
                self.fetch50 = None

            if "FETCH_90" in self.footprint.keys():
                self.fetch90 = np.percentile(
                    self.footprint["FETCH_90"][self.footprint["FETCH_90"] > 0], 50
                )
            else:
                self.fetch90 = None
        else:
        ...
        self.lat = float(
            self.site_info.loc[self.site_info["VARIABLE"] == "LOCATION_LAT"][
                "DATAVALUE"
            ].values
        )
        self.lon = float(
            self.site_info.loc[self.site_info["VARIABLE"] == "LOCATION_LONG"][
                "DATAVALUE"
            ].values
        )
        try:
            self.elev = float(
                self.site_info.loc[self.site_info["VARIABLE"] == "LOCATION_ELEV"][
                    "DATAVALUE"
                ].values[0]
            )
        except:
    ...
    def add_tower_data(self, missing_displacement_heights_dict=None):
        ...
        if self.need_footprint_variables:
            ...
            group_ids_nee_entries = (
                variable_info["GROUP_ID"]
                .iloc[variable_info.index[variable_info["DATAVALUE"] == "NEE_VUT_REF"]]
                .values
            )
            # get indices of the groups
            indices_group_id = variable_info.index[
                variable_info["GROUP_ID"].isin(group_ids_nee_entries)
            ]
            # get indices that describe heigth parameter
            indices_height_parameter = variable_info.index[
                variable_info["VARIABLE"] == "VAR_INFO_HEIGHT"
            ]
            ...
            indices_parameter_date = variable_info.index[
                variable_info["VARIABLE"] == "VAR_INFO_DATE"
            ]
            # get wanted dates
            ...
            if not (
                dates_height_measurement == np.sort(dates_height_measurement)
            ).all():
            ...
            for idx_height, height in enumerate(heights):
                mask = idata["TIMESTAMP_END"] >= float(
                    dates_height_measurement[idx_height]
                )
            ...
            if self.icos_2025:
                fluxes_file_data = pd.read_csv(
                    ...
                    usecols=lambda x: x in wanted_fluxes_variables,
                    on_bad_lines="skip",
                )
            else:
                fluxes_file_data = pd.read_csv(
            ...
            print(
                "not available in fluxes file:",
                [
                    col
                    for col in wanted_fluxes_variables
                    if col not in fluxes_file_data.columns
                ],
            )
        idata.rename({self.ssrd_key: "ssrd", self.t2m_key: "t2m"}, inplace=True, axis=1)
        tf = TimezoneFinder()
        ...
        if self.need_footprint_variables:
            ...
            if (self.tstart is not None) & (self.tstop is not None):
                mask = (fluxes_file_data["datetime_utc"] >= self.tstart) & (
                    fluxes_file_data["datetime_utc"] < self.tstop
                )
                fluxes_file_data = fluxes_file_data[mask]
            # merge FLUXNET and FLUXES data
            flux_data = flux_data.merge(
                fluxes_file_data, how="inner", on=["datetime_utc", "TIMESTAMP_END"]
            )
            ...
            if "MO_LENGTH" in flux_data.columns:
                ...
                for idx in range(len(dates_height_measurement) + 1):
                    if idx == 0:
                        ...
                        lower_lim = float(dates_height_measurement[idx - 1])
                        upper_lim = float(dates_height_measurement[idx])
                    mask = (flux_data["TIMESTAMP_END"] >= lower_lim) & (
                        flux_data["TIMESTAMP_END"] <= upper_lim
                    )
                    # print(mask)
                    try:

                        flux_data.loc[mask, "z_footprint"] = np.max(
                            [
                                0,
                                np.nanmean(
                                    np.where(
                                        flux_data.loc[mask, "ZL"] > -9999,
                                        flux_data.loc[mask, "ZL"],
                                        np.nan,
                                    )
                                    * np.where(
                                        flux_data.loc[mask, "MO_LENGTH"] > -9999,
                                        flux_data.loc[mask, "MO_LENGTH"],
                                        np.nan,
                                    )
                                ),
                            ]
                ...
                missing_displacement_heights_dict is not None
                and self.site_name in missing_displacement_heights_dict.keys()
            ):
                flux_data["z_footprint"] = (
                    flux_data["z_measurement"]
                    - missing_displacement_heights_dict[self.site_name]
                )
            else:
                flux_data["z_footprint"] = flux_data["z_measurement"]
            flux_data["z_displacement"] = (
                flux_data["z_measurement"] - flux_data["z_footprint"]
            )
