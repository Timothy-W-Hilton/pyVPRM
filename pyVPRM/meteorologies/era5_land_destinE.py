...
class met_data_handler(met_data_handler_base):
    ...
    def __init__(
        ...
        if PAT is None:
            print(
                "Need to set the access token via the PAT argument. Check https://platform.destine.eu/."
            )
            return
        super().__init__()
        self.PAT = PAT
    ...
    def load_ds(self):
        self.ds = (
            xr.open_dataset(
                "https://edh:{}@data.earthdatahub.destine.eu/era5/reanalysis-era5-land-no-antartica-v0.zarr".format(
                    self.PAT
                ),
                chunks={},
                engine="zarr",
    ...
    def reduce_along_lonlat(self, lon, lat, interp_method="nearest"):
        ...
        self.ds = self.ds.interp(
            lon=("lon", lon), lat=("lat", lat), method=interp_method
        )
        return
        ...
        if lonlat is None:
            ...
            if isinstance(lon, list) | isinstance(lon, np.ndarray):
                if self.rearranged is False:
                    lon = [map_function_inv(i) for i in lon]
                return tmp.interp(
                    lon=("lon", lon), lat=("lat", lonlat[1]), method=interp_method
                )
            else:
                lon = lonlat[0]
                if self.rearranged is False:
                    lon = map_function_inv(lon)
                return tmp.interp(lon=lon, lat=lonlat[1], method=interp_method)
